...
class Model_asi(nn.Module):
    def __init__(self, t, hidden_layers_width=[100], input_size=20, num_classes: int = 1000, act_layer: nn.Module = nn.ReLU()):
        super(Model_asi, self).__init__()
        self.num_classes = num_classes
        self.input_size = input_size
        self.hidden_layers_width = hidden_layers_width
        self.t = t
        ...
        # 创建两个网络
        self.net1 = self._create_network()
        self.net2 = self._create_network()
    ...
    def _create_network(self):
        ...
        self.layers_width = [self.input_size] + self.hidden_layers_width
        for i in range(len(self.layers_width) - 1):
            layers += [nn.Linear(self.layers_width[i], self.layers_width[i+1]), self.act_layer]
        layers += [nn.Linear(self.layers_width[-1], self.num_classes)]
    ...
    def _initialize_weights(self) -> None:
        for obj1, obj2 in zip(self.net1.modules(), self.net2.modules()):
            if isinstance(obj1, (nn.Linear, nn.Conv2d)):
                if obj1 == self.net1[-1]:
                    nn.init.normal_(obj1.weight.data, 0, 1 / self.hidden_layers_width[0]**(self.t))
                    # nn.init.normal_(obj2.weight.data, 0, 1 / self.hidden_layers_width[0]**(self.t))
                    obj2.weight.data = copy.deepcopy(-obj1.weight.data)
                else:
                    nn.init.normal_(obj1.weight.data, 0, 1 / self.hidden_layers_width[0]**(self.t))
                    obj2.weight.data = copy.deepcopy(obj1.weight.data)
                if obj1.bias is not None:
                    if obj1 == self.net1[-1]:
                        nn.init.normal_(obj1.bias.data, 0, 1 / self.hidden_layers_width[0]**(self.t))
                        obj2.bias.data = copy.deepcopy(-obj1.bias.data)
                    else:
                        nn.init.normal_(obj1.bias.data, 0, 1 / self.hidden_layers_width[0]**(self.t))
                        obj2.bias.data = copy.deepcopy(obj1.bias.data)
...
class sin_100_torch(nn.Module):  # Custom activation function
    def __init__(self):
        super(sin_100_torch, self).__init__()
...
class ricker_torch(nn.Module):  # Custom activation function
    ...
    def forward(self, x):
        ...
        a=0.1
        return (2/(30*a))*(math.pi**0.25) * (1 - (x/a)**2) * torch.exp(-0.5*(x/a)**2)
...
def get_act_func(act_func):
    ...
    if act_func == 'tanh':
        ...
        return hat_torch()
    elif act_func == 'ricker':
        return ricker_torch()
    elif act_func == 'sin_100':
        return sin_100_torch()
    else:
